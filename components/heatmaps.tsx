...
import { format, eachDayOfInterval, subDays, startOfDay } from "date-fns"
import { useState } from "react"
type Session = { date: string | Date; minutes: number }
type Props = { sessions: Session[] }
...
export function Heatmap({ sessions }: Props) {
  ...
  const levelStyles = {
    0: { bg: "var(--dot-empty)", shadow: "none", scale: "scale-75" },
    1: { bg: "var(--dot-l1)", shadow: "0 0 4px var(--dot-l1-glow)", scale: "scale-90" },
    2: { bg: "var(--dot-l2)", shadow: "0 0 6px var(--dot-l2-glow)", scale: "scale-100" },
    3: { bg: "var(--dot-l3)", shadow: "0 0 8px var(--dot-l3-glow)", scale: "scale-100" },
    4: { bg: "var(--dot-l4)", shadow: "0 0 12px var(--dot-l4-glow)", scale: "scale-100" },
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={stat.label} className="border border-border rounded-xl px-4 py-3 bg-card">
...
function calculateStreak(minutesByDay: Record<string, number>, today: Date): number {
  let streak = 0
  let current = today
  while (true) {
    const key = format(current, "yyyy-MM-dd")
